...
namespace 名字生成器
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            var nameFile=this.textBox_surname.Text;
            var wordFile = this.textBox_word.Text;
            var names = GetFile(nameFile);
            var words = GetFile(wordFile);
            foreach(char name in names)
            {
                StringBuilder str = new StringBuilder();
                foreach(char word in words)
                {
                    if (this.checkBox_three.Checked)
                    {
                        foreach(char _word in words)
                        {
                            var userName = name.ToString() + word.ToString() + _word.ToString();
                            str.AppendLine(userName);
                        }
                    }
                    if (this.checkBox_two.Checked)
                    {
                        var userName = name.ToString() + word.ToString();
                        str.AppendLine(userName);
                    }
                    else
                    {
        ...
        {
            mutex.WaitOne();
            using (StreamWriter sw = new StreamWriter(CheckFile("名字.txt"), true, Encoding.Default))
            {
        ...
        //汉子转换为拼音
        public string ConvertWordToPY(char c)
        {
